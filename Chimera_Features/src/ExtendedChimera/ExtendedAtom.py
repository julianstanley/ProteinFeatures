...
class ExtendedResidue:
    ...
    def __init__(
        ...
        # AAind attributes
        self.aaind_reactivity = reactivity_index_dict.get(
            self.residue_1_letter, None)
        ...
        if self.area_sas != self.area_sas:
            self.area_sas = 0

        # Misc. Attributes
        self.hydroxyl_constant = OHrxnConst_index_dict.get(
            self.residue_1_letter, None)
        self.charge = charge_index_dict.get(self.residue_1_letter, None)
        self.hydrophobicity = kd_hydrophobicity.get(
            self.residue_1_letter, None)
        self.reactivity = reactivity_index_dict.get(
            self.residue_1_letter, None)
        ...
        if depths is None:
            ...
            if (
                "{},{}".format(self.number.split(
                    ".")[0], self.residue_1_letter)
                in depths
            ):
                self.depth = depths[
                    "{},{}".format(self.number.split(
        ...
        for metal in self.all_metals:
            for atom in self.atoms:
                ...
                distance = chimera.distance(
                    atom.xformCoord(),
                    chimera.Point(
                        metal_coords[0], metal_coords[1], metal_coords[2]),
                )
